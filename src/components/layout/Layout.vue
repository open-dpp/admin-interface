              ...
              <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-white px-6 pb-4">
                ...
                <nav class="flex flex-1 flex-col">
                  <ul class="flex flex-1 flex-col gap-y-7" role="list">
                    <li>
                      <ul class="-mx-2 space-y-1" role="list">
                        <li v-for="item in navigation" :key="item.name">
                          <router-link
                              :class="[item.to === route.path ? 'bg-gray-50 text-GJDarkGreen' : 'text-gray-700 hover:bg-gray-50 hover:text-GJDarkGreen', 'group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6']"
                              :to="item.to">
                            <component :is="item.icon"
                                       :class="[item.to === route.path ? 'text-GJDarkGreen' : 'text-gray-400 group-hover:text-GJDarkGreen', 'h-6 w-6 shrink-0']"
                                       aria-hidden="true"/>
                            {{ item.name }}
                          </router-link>
                        </li>
                    ...
                    <li>
                      <div class="text-xs font-semibold leading-6 text-gray-400">Schnellzugriff</div>
                      <ul class="-mx-2 mt-2 space-y-1" role="list">
                        <li v-for="team in teams" :key="team.name">
                          <router-link
                              :class="[team.current ? 'bg-gray-50 text-GJDarkGreen' : 'text-gray-700 hover:bg-gray-50 hover:text-GJDarkGreen', 'group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6']"
                              :to="team.to">
                            <span
                                :class="[team.current ? 'border-GJDarkGreen text-GJDarkGreen' : 'border-gray-200 text-gray-400 group-hover:border-GJDarkGreen group-hover:text-GJDarkGreen', 'flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border bg-white text-[0.625rem] font-medium']">{{
                                team.initial
                              }}</span>
              ...
              <MenuButton class="-m-1.5 flex items-center p-1.5">
                ...
                <span class="hidden lg:flex lg:items-center">
                  <span aria-hidden="true"
                        class="ml-4 text-sm font-semibold leading-6 text-gray-900">Florian Bieck</span>
