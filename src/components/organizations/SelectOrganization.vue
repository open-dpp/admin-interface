<template>
  <Listbox as="div" :model-value="indexStore.selectedOrganization" @update:model-value="(org) => setOrganization(org.id)">
    <ListboxLabel class="block text-sm/6 font-medium text-gray-900">Organisation wählen</ListboxLabel>
    <div class="relative flex flex-row gap-2">
      <div v-if="organizationsStore.organizations.length > 0" class="flex flex-grow mt-2">
        <ListboxButton class="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-1.5 pl-3 pr-2 text-left text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
          <span class="col-start-1 row-start-1 truncate pr-6">{{ nameOfSelectedOrganization }}</span>
        ...
        <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
          ...
          >
            <ListboxOption as="template" v-for="organization in organizationsStore.organizations" :key="organization.id" :value="organization" v-slot="{ active, selected }">
              <li :class="[active ? 'bg-indigo-600 text-white outline-none' : 'text-gray-900', 'relative cursor-default select-none py-2 pl-8 pr-4']">
                <span :class="[selected ? 'font-semibold' : 'font-normal', 'block truncate']">{{ organization.name }}</span>
      ...
      <router-link to="/organizations/create">
        <button type="button" class="flex items-center p-2 rounded mt-2 bg-indigo-500 text-white">
          <PlusCircleIcon class="size-5" />
          <span v-if="organizationsStore.organizations.length === 0" class="text-md pl-1">Organisation erstellen</span>
        </button>
      </router-link>
      <router-link v-if="false" to="/organizations">
        <button type="button" class="flex items-center p-2 rounded mt-2 bg-indigo-700 text-white">
...
import { CheckIcon, PlusCircleIcon, ListBulletIcon } from '@heroicons/vue/20/solid'
...
const nameOfSelectedOrganization = computed(() => {
  if (indexStore.selectedOrganization) {
    return organizationsStore.organizations.find(org => org.id === indexStore.selectedOrganization)?.name;
  }
  return 'Auswählen';
});
