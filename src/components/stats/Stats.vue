      ...
      <div v-for="item in stats" :key="item.id"
        ...
        <dt>
          <div class="absolute rounded-md bg-indigo-500 p-3">
            <component :is="item.icon" aria-hidden="true" class="h-6 w-6 text-white"/>
          </div>
        ...
        <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
          ...
          <p :class="[item.changeType === 'increase' ? 'text-green-600' : 'text-red-600', 'ml-2 flex items-baseline text-sm font-semibold']">
            <ArrowUpIcon v-if="item.changeType === 'increase'" aria-hidden="true"
                         class="h-5 w-5 flex-shrink-0 self-center text-green-500"/>
            <ArrowDownIcon v-else aria-hidden="true" class="h-5 w-5 flex-shrink-0 self-center text-red-500"/>
